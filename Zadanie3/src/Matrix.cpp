...
const double Matrix::determinant(){
  ...
  auto dataCp = *this;
  int swapCount =0;
    if(debug)  cout <<endl<<"Before" <<endl<< dataCp;
    // Swaping columns
    for(int i = 0; i<SIZE;i++){
      if(dataCp.isZero(i,i)){
        ...
        do{
         if(debug) cout << "Element "<< i << " " << j << " is " << dataCp.value[i][j] << endl;
          if(!dataCp.isZero(i,j)){
           if(debug) cout << "Column " << j << " seems to be a good replacement! Proceeding to swap..." <<endl;
            swap =0;
            swapCount++;
            for(int k=0;k<SIZE;k++) std::swap(dataCp.value[i][k], dataCp.value[j][k]);
          }
          j++;
          if(j == SIZE){
            j=0;
    ...
    if(debug)  cout << endl << "After" <<endl<< dataCp << endl << "Swap count: " << swapCount <<endl;
    // Check if all diagonals arent nulls
    for (int i =0;i<SIZE;i++){
      if(dataCp.isZero(i,i)){ //if yes, proceed another swaping round
       if(debug)   cout << "There is still zero in diagonals! Another round" << endl;
          for(int j =0; j<SIZE; j++){ 
            if(!dataCp.isZero(i,j)){  // Check if whole column isnt filled with nulls
              if(swapCount % 2 == 0){ // Number of swaps changes positivity of determinant
                return dataCp.determinant(); // even? Dont change
              }else{ // odd? Change the minus
                return !dataCp.determinant();
              }
            } 
          }
