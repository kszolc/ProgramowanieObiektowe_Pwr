...
int comComp(){
    ...
    Vector<Com> constTerm;
    Vector<Com> matrix[SIZE];
    Vector<Com> result;
    ...
    for(int i = 0;i<SIZE;i++){
        for(int j =0;j<SIZE;j++){
            cout <<" ("<< matrix[j].value[i].re<<showpos<<matrix[j].value[i].im<<noshowpos << ") ";
        }
        cout << endl;
    }
    ...
    */
    ...
    cout << "Trochę wstyd się przyznać, ale nawet na papierze nie mam pojęcia jak bym miał rozwiązać macierz 5 stopnia z liczbami zespolonymi...";
    cout << endl;
    return 1;
